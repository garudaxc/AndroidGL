...
MessageQueue::~MessageQueue()
{
	...
	for ( ; ; )
	{
		...
		GLog.LogInfo( "%p:~MessageQueue: still on queue: %s", this, msg );
		free( (void *)msg );
	}
...
{
	GLog.LogInfo( "%p:MessageQueue shutdown", this );
	shutdown = true;
}
...
bool MessageQueue::PostMessage( const char * msg, bool synced, bool abortIfFull )
{
	if ( shutdown )
	{
		GLog.LogInfo( "%p:PostMessage( %s ) to shutdown queue", this, msg );
		return false;
	}
	if ( debug )
	{
		GLog.LogInfo( "%p:PostMessage( %s )", this, msg );
	}
	...
	if ( tail - head >= maxMessages )
	{
		...
		if ( abortIfFull )
		{
			GLog.LogInfo( "MessageQueue overflow" );
			for ( int i = head; i <= tail; i++ )
			{
				GLog.LogInfo( "%s", messages[i % maxMessages].string );
			}
			GLog.LogError( "Message buffer overflowed" );
		}
		return false;
	}
...
const char * MessageQueue::GetNextMessage()
{
	...
	if ( debug )
	{
		GLog.LogInfo( "%p:GetNextMessage() : %s", this, msg );
	}
...
void MessageQueue::SleepUntilMessage()
{
	...
	if ( debug )
	{
		GLog.LogInfo( "%p:SleepUntilMessage() : sleep", this );
	}
	...
	if ( debug )
	{
		GLog.LogInfo( "%p:SleepUntilMessage() : awoke", this );
	}
}
...
void MessageQueue::ClearMessages()
{
	if ( debug )
	{
		GLog.LogInfo( "%p:ClearMessages()", this );
	}
	for ( const char * msg = GetNextMessage(); msg != NULL; msg = GetNextMessage() )
	{
		GLog.LogInfo( "%p:ClearMessages: discarding %s", this, msg );
		free( (void *)msg );
	}
}
